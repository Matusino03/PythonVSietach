...
#           EXIT|nick
#           USERS|nick1,nick2,nick3... #server posiela zoznam pouzivatelov na WHO
#           WHO|nick #klient pyta zoznam prihlasenych
...
class ChatProtocol:
    ...
    def parse(self, bin_msg : bytes, user_list : list, client_sock : s.socket, lock : Lock):
        ...
        if len(list_msg_parts) > 2:
            msg = list_msg_parts[2]

        if list_msg_parts[0] == "LOGIN":
            lock.acquire()
            user_list.append(nick)
            lock.release()
            print("Client '{}' has connected to server".format(nick))

        elif list_msg_parts[0] == "WHO":
            print("Client '{}' has requested list of users".format(nick))
            ...
            lock.release()
            print("Client '{}' has disconnected from server".format(nick))

        elif list_msg_parts[0] == "SENDMSG":
            print("Client '{}' msg: {}".format(nick, msg))

def handleClient(client_sock, lock):
    protocol = ChatProtocol("", lock)
    while True:
        client_msg = client_sock.recv(MSG_SIZE)
        protocol.parse(client_msg, USERS, client_sock)
...
while True:
    ...
    t = Thread(target=handleClient, args=[client_sock, lock])
    t.run()
